...
/* ════════════════════════════════════
   6. LEAD MAGNET CTA
════════════════════════════════════ */
function LeadMagnetCTA({ onNavigate }) {
  const w = useWidth(); const isMobile = w < 640;
  const ref = useReveal();
  return (
    <HomepageFinalCtaShell isMobile={isMobile}>
      <div className="reveal" ref={ref} style={{ width: "100%" }}>
        <div style={{ display:"inline-flex", alignItems:"center", gap:7, background:"rgba(245,166,35,.12)", border:"1px solid rgba(245,166,35,.28)", borderRadius:100, padding:"5px 14px", marginBottom: isMobile ? 16 : 28 }}>
        ...
        <h2 style={{
          ...
          lineHeight:1.1, letterSpacing:"-.04em", paddingBottom:"0.12em",
          background:"linear-gradient(95deg,#fff 0%,#fff 30%,#F5D060 62%,#F5A623 100%)",
          WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text",
        ...
        <HomepageFinalCtaButton
          isMobile={isMobile}
          label="Download Now →"
          href={getProductGatedDownloadHref("vdd")}
        />
      </div>
  ...
  return (
    <>
      ...
      <main>
        ...
        <LeadMagnetCTA onNavigate={onNavigate} />
      </main>
