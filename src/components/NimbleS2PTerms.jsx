...
function VDDFooter({ onNavigate }) {
  ...
  const dots = [
    { left:"12%", top:"18%", size:1.5, op:.65 }, { left:"67%", top:"12%", size:1, op:.55 },
    { left:"83%", top:"58%", size:1.5, op:.50 }, { left:"34%", top:"72%", size:1, op:.60 },
    { left:"55%", top:"40%", size:1,   op:.45 },
  ];
  return (
    <footer style={{ background:"oklch(0.08 0.02 270)", position:"relative", overflow:"hidden" }}>
      <div style={{ position:"absolute", inset:0, pointerEvents:"none", zIndex:0, background:`radial-gradient(ellipse 55% 60% at 10% 20%, oklch(0.25 0.08 280 / 0.35), transparent 70%), radial-gradient(ellipse 50% 55% at 88% 78%, oklch(0.22 0.10 270 / 0.30), transparent 70%)` }} />
      {dots.map((d,i) => (
        <div key={i} style={{ position:"absolute", left:d.left, top:d.top, zIndex:1, pointerEvents:"none", width:d.size, height:d.size, borderRadius:"50%", background:"radial-gradient(circle, oklch(0.85 0.08 280 / 0.9), transparent 70%)", opacity:d.op }} />
      ))}
      <span style={{ position:"absolute", inset:"-10%", pointerEvents:"none", zIndex:1, filter:"blur(60px)", willChange:"transform", background:"radial-gradient(50% 60% at 30% 50%, oklch(0.60 0.22 280 / 0.52), transparent 70%)", animation:"drift-x 30s ease-in-out infinite" }} />
      <span style={{ position:"absolute", inset:"-10%", pointerEvents:"none", zIndex:1, filter:"blur(60px)", willChange:"transform", background:"radial-gradient(45% 55% at 70% 60%, oklch(0.55 0.24 260 / 0.49), transparent 70%)", animation:"drift-x-rev 40s ease-in-out infinite" }} />
      <div style={{ maxWidth:1080, margin:"0 auto", position:"relative", zIndex:10, padding: isMobile ? "47px 20px 0" : "62px 5vw 0" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10, borderTop:"1px solid rgba(255,255,255,.07)", padding: isMobile ? "21px 0 31px" : "26px 0 36px" }}>
          <div style={{ fontSize:12, color:"rgba(255,255,255,.22)", fontFamily:"var(--fb)" }}>© 2025 NimbleS2P. All rights reserved.</div>
