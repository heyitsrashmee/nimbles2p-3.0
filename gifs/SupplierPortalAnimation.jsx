...
export default function SupplierPortalAnimation() {
  ...
  function sleep(ms) {
    return new Promise(r => { loopRef.current = setTimeout(r, ms); });
  }
  ...
  const S = {
    shell: {
      width:"100%", height:"100%", minHeight:0, background:"#F8FAFC",
    ...
    tabs: {
      ...
      background:"#fff", padding:"0 14px", flexShrink:0,
    },
    tab: (act) => ({
      padding:"9px 11px", fontSize:11, fontWeight: act ? 700 : 500,
      color: act ? "#4F46E5" : "#64748B",
      borderBottom: act ? "2px solid #4F46E5" : "2px solid transparent",
      display:"flex", alignItems:"center", gap:4,
    }),
    tabDot: { width:5, height:5, borderRadius:"50%", background:"currentColor" },
    body: { display:"grid", gridTemplateColumns:"140px 1fr", flex:1, minHeight:0, overflow:"hidden" },
    list: { borderRight:"1px solid #E2E8F0", padding:"10px 0", background:"#FAFBFC", overflow:"hidden" },
    listHd: { fontSize:9, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", color:"#94A3B8", padding:"0 12px 6px" },
    listRow: (act) => ({
      padding:"8px 12px", fontSize:11, fontWeight: act ? 600 : 500,
      color: act ? "#4F46E5" : "#0F172A",
      borderLeft: `3px solid ${act ? "#4F46E5" : "transparent"}`,
      background: act ? "#EEF2FF" : "transparent",
      lineHeight:1.35,
    }),
    listSub: { fontSize:9, color:"#94A3B8", fontWeight:400, marginTop:1 },
    badge: (type) => ({
      display:"inline-block", fontSize:8, fontWeight:700, padding:"1px 5px",
    ...
    panelTitle: { fontSize:12, fontWeight:700, color:"#0F172A" },
    panelSub: { fontSize:10, color:"#64748B", marginTop:1 },
    chip: { fontSize:9, fontWeight:700, padding:"2px 8px", borderRadius:100, background:"#FEF3C7", color:"#92400E" },
  ...
  const fldStyle = (key) => ({
    ...
    width:"100%", minHeight:30, resize:"none",
          ...
          <div style={S.screens}>
            ...
            <div style={S.screen(phase === PHASE.EMPTY, 20)}>
              <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:8, textAlign:"center" }}>
                ...
                <div style={{ fontSize:11, color:"#64748B", maxWidth:160, lineHeight:1.5 }}>
              ...
              <div style={{ flex:1, display:"flex", flexDirection:"column", gap:8, overflowY:"auto" }}>
                {FIELDS.map(f => (
                  <div key={f.key} style={{ display:"flex", flexDirection:"column", gap:3 }}>
                    <label style={{ fontSize:10, fontWeight:600, color:"#64748B", letterSpacing:".03em" }}>
                      {f.label}
                    </label>
                    {f.key === "desc"
                      ? <textarea readOnly style={{ ...fldStyle(f.key), minHeight:54, fontSize:10, lineHeight:1.5 }} value={fieldVals[f.key] || ""} />
            ...
            <div style={S.screen(phase === PHASE.PROCESSING && slideIn, 28)}>
              <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-start", paddingTop:18, gap:14 }}>
                ...
                <div style={{ fontSize:12, fontWeight:600, color:"#0F172A", display:"flex", alignItems:"center" }}>
            ...
            <div style={S.screen(phase === PHASE.RESOLUTION && slideIn, 28)}>
              <div style={{ flex:1, display:"flex", flexDirection:"column", gap:10, overflowY:"auto" }}>
                ...
                <div style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
                  ...
                  <div style={{ background:"#EEF2FF", border:"1px solid #C7D2FE", borderRadius:"0 9px 9px 9px", padding:"9px 11px", fontSize:11.5, color:"#0F172A", lineHeight:1.6, flex:1 }}>
                ...
                <div style={{ background:"#FFFBEB", border:"1px solid #FDE68A", borderRadius:9, padding:"10px 12px" }}>
                  <div style={{ fontSize:10, fontWeight:700, color:"#D97706", letterSpacing:".05em", textTransform:"uppercase", marginBottom:7, display:"flex", alignItems:"center", gap:4 }}>
                    ⚠ Budget Variance
                  </div>
                  {[
                ...
                {/* Action rows */}
                <div style={{ fontSize:9, fontWeight:700, letterSpacing:".08em", textTransform:"uppercase", color:"#94A3B8" }}>
                  Recommended Actions
                </div>
                {[
                  ...
                  <div key={i} style={{
                    ...
                    borderRadius:7, padding:"8px 10px", cursor:"default",
                    ...
                    <div style={{ fontSize:11, fontWeight:600, color:"#0F172A", flex:1 }}>{txt}</div>
              ...
              <div style={{
                ...
                <div>
                  <div style={{ fontSize:12, fontWeight:700 }}>Action submitted successfully</div>
                  <div style={{ fontSize:10, opacity:.85 }}>PO Amendment request sent to Finance team</div>
                </div>
              </div>
            </div>
